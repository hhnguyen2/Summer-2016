...
gen_sim.data <- function(){
  ...
  #omega <- c(-0.41,0.85,-0.91,0.20,0.50)    # omega : logistic parameters for A|Z,X
  omega <- c(-0.15,0.85,-0.30,0.75,0.60)
  #mu <<- c(1.62,1.11,1.76,-1.24,-1.16,-1.33,-1.97) # mu: logistic paramters for A, delta, x
  #my.gamma <- c(0.65,-0.19,-0.24,0.14,-0.14) # gamma: used to coerce Pr.A.zx between (0,1)
  my.gamma <- c(0.59,-0.16,0.06,-0.13,-0.23)
  ...
  mu <<- c(1.62,0,0.58,-1.54,-0.65,0.55,1.26,-0.77)
  Yi <-  mu[1]*A + mu[3] + rowSums(t(t(xi)*mu[4:(length(mu)-1)])) + mu[length(mu)]*zi
...
gen_f.zx <- function(alpha_hat,sim.data){
  # Extract xi and zi
  xi <- sim.data[,c(1,
                      grep("xi",colnames(sim.data)))] # 1,x1,...,xn
  zi <- sim.data$zi
